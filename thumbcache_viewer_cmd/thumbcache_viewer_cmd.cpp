...
int wmain( int argc, wchar_t *argv[] )
{
	...
	int directory_length = wcslen( name );
	...
	do
	{
		...
		if ( hFile != INVALID_HANDLE_VALUE )
		{
			...
			if ( output_html )
			{
				...
				{
					hFile_html = CreateFileA( "Report.html", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
				}
				else
				{
					already_open = true;
			...
			if ( output_csv )
			{
				...
				{
					hFile_csv = CreateFileA( "Report.csv", GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
				}
				else
				{
					already_open = true;
