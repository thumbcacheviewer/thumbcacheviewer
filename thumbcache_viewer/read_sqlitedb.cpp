...
void CleanupSQLiteInfo()
{
	...
	while ( node != NULL )
	{
		...
		if ( pi != NULL )
		{
			if ( pi->sei != NULL && pi->sei->count == 0 )	// Shared info is cleaned up when the main list is cleaned.
			{
...
argv[ 4 ] = VariantType
*/
int CheckPropertyInfoCallback( void *arg, int argc, char **argv, char ** /*azColName*/ )
{
	PROPERTY_INFO_STATUS *pis = ( PROPERTY_INFO_STATUS * )arg;
	if ( pis != NULL && argc == 5 )
	{
		...
		switch ( Id_num )
		{
			case 438:	// 14F-System_FileAttributes
			{
				pis->file_attributes = strtoul( val, NULL, 10 );
			}
			break;

			case 434:	// 4392-System_FileExtension
			{
				int val_length = MultiByteToWideChar( CP_UTF8, 0, val, -1, NULL, 0 );	// Include the NULL terminator.
				pis->file_extension = ( wchar_t * )malloc( sizeof( wchar_t ) * val_length );
				MultiByteToWideChar( CP_UTF8, 0, val, -1, pis->file_extension, val_length );
			}
			break;

			case 33:	// 4447-System_ItemPathDisplay
			{
				int val_length = MultiByteToWideChar( CP_UTF8, 0, val, -1, NULL, 0 );	// Include the NULL terminator.
				pis->item_path_display = ( wchar_t * )malloc( sizeof( wchar_t ) * val_length );
				MultiByteToWideChar( CP_UTF8, 0, val, -1, pis->item_path_display, val_length );
			}
			break;
		}
	}
...
int WINAPI	s_CheckPropertyInfoCallback( void *arg, int argc, char **argv, char **azColName ) { return CheckPropertyInfoCallback( arg, argc, argv, azColName ); }
int WINAPIV	c_CheckPropertyInfoCallback( void *arg, int argc, char **argv, char **azColName ) { return CheckPropertyInfoCallback( arg, argc, argv, azColName ); }
	...
	if ( fi != NULL && argc == 5 )
	{
		...
		switch ( Type )
		{
			...
			case VT_UI4:
			{
				...
				if ( pi->property_name_length == 21 && wcscmp( ei->sei->windows_property, L"System_FileAttributes" ) == 0 )
				{
					ei->property_value = GetFileAttributesStr( num_val );
				}
				else if ( ( pi->property_name_length == 17 && wcscmp( ei->sei->windows_property, L"System_SFGAOFlags" ) == 0 ) ||
						  ( pi->property_name_length == 28 && wcscmp( ei->sei->windows_property, L"System_Link_TargetSFGAOFlags" ) == 0 ) )
				{
			...
			case VT_UI8:
			{
				...
				if ( pi->property_name_length == 23 && wcscmp( ei->sei->windows_property, L"System_ThumbnailCacheId" ) == 0 )
				{
					ei->property_value = ( wchar_t * )malloc( sizeof( wchar_t ) * 17 );
					swprintf_s( ei->property_value, 17, L"%016llx", ui8_val );
				}
				else if ( pi->property_name_length == 11 && wcscmp( ei->sei->windows_property, L"System_Size" ) == 0 )
				{
			...
			case VT_BLOB:
			{
				if ( pi->property_name_length == 11 && wcscmp( ei->sei->windows_property, L"System_Kind" ) == 0 )
				{
				...
				/*else if ( ( pi->property_name_length == 16 && wcscmp( ei->sei->windows_property, L"InvertedOnlyPids" ) == 0 ) ||
						  ( pi->property_name_length == 15 && wcscmp( ei->sei->windows_property, L"InvertedOnlyMD5" ) == 0 ) )
				{
					// Output hex values.
